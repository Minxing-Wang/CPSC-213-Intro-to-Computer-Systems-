...
void interrupt_service_routine() {
 struct pending_read* pendr = queue_dequeue (&prq);
 pendr->handler(pendr->buf, pendr->blockno); 
}
...
void run (int num_blocks) {
  ...
  for (int blockno = 0; blockno < num_blocks; blockno++) {
    init_pending_read(&pr[blockno], &buf[blockno][0], blockno, handle_read);
    queue_enqueue(&prq, &pr[blockno]);
    disk_schedule_read(&buf[blockno][0], sizeof (buf[blockno]), blockno); 
  }
  disk_wait_for_reads();
}
